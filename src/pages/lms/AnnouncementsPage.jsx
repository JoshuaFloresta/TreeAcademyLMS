...
function ComposeForm({ courses, onDone }) {
  ...
  const submit = async (event) => {
    ...
    setError('')
    try { await mutation.mutateAsync(); toast.success('Announcement posted.'); setValues({ courseId: '', title: '', body: '', pinned: false }); setOpen(false); onDone() } catch (e) { setError(e.message) }
  }
  if (!open) return <button type="button" className="button button-primary" onClick={() => setOpen(true)}><Plus size={16} /> New announcement</button>
  return <form className="announce-form" onSubmit={submit}>
    ...
    <div className="announce-form-foot">
      ...
      <div className="builder-lesson-actions"><button className="button button-primary button-compact" disabled={mutation.isPending}>Post announcement</button><button type="button" className="button button-ghost button-compact" onClick={() => setOpen(false)}>Cancel</button></div>
    </div>
...
export default function AnnouncementsPage({ role }) {
  ...
  return <>
    <div className="page-title-row">
      ...
      {isStaff && courses.length > 0 && <ComposeForm courses={courses} onDone={refresh} />}
    </div>
    <div className="announce-list">
      {isLoading ? <p className="operations-note">Loading announcements…</p>
        ...
        : announcements.map((announcement) => <article className={`announce-card ${announcement.pinned ? 'pinned' : ''}`} key={announcement.id}>
          <div className="announce-head">
            ...
            <span className="announce-meta">{announcement.authorName} · {when(announcement.createdAt)}{isStaff && <button type="button" className="builder-danger" onClick={() => remove(announcement)} aria-label="Delete announcement"><Trash2 size={14} /></button>}</span>
          </div>
