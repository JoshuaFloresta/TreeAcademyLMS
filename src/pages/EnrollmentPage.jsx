...
export default function EnrollmentPage() {
  ...
  const content = pathwayBlockedMessage && step === 1 && !application
    ...
    : step === 2 ? <DocumentStep enrollmentId={application?.id} type={documentType} applicant={application} application={intake} onSubmit={(payload) => submitDocument(documentType, payload)} onBack={() => setStep(1)} submitting={busy} error={formError} />
      : step === 3 ? <PaymentStep amount={application?.amount} currency={application?.currency} upfrontAmount={upfrontAmount} pricing={pricing} pathwayId={pathway.id} discount={application?.discount} onApplyVoucher={applyVoucher} onRemoveVoucher={removeVoucher} onPay={launchPayment} message={paymentMessage} onBack={() => setStep(2)} loading={busy} />
